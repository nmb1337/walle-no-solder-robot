...
## Current cart override

The current cart uses two `43MM轮子一个` N20 D-axis rubber wheels. Treat this
as the active wheel interface: 43 mm outside diameter, N20 D bore and a
maximum 19 mm width. It supersedes the older 42 mm search reference below.
...
| --- | --- | --- | --- |
| 1 | Waveshare ESP32-S3-Touch-LCD-3.5-C, with case + OV5640 camera | Official case 95.11 x 63.67 x 14.10 mm; ESP32-S3, MEMS mic, ES8311 codec, USB-C and GPIO | `微雪 ESP32-S3-Touch-LCD-3.5-C 带外壳 摄像头` |
| 1 | Supplied OV5640 camera carrier | Seller must confirm carrier envelope <= 30 x 30 mm, FPC contact side and cable length | `OV5640 摄像头 模块 排线` |
...
| 1 | 5 V / 5-8 A fixed buck converter, screw terminals, 2S-compatible input | Feeds PCA9685 servo rail, ESP32 DevKitC and the touch-LCD board through USB-C. The listing must explicitly support 6.0-8.4 V input; 3 A is not sufficient. |
| 1 | 6.0 V / 3 A fixed buck converter, screw terminals, 2S-compatible input | Feeds only TB6612 `VM` and the two N20 motors. A 2S pack must not be connected directly to 6 V motors. |
